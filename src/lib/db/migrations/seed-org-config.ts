...
export function seedOrgConfig(
  ...
  const seeds: ConfigSeedRow[] = [
    ...
    {
      key: "backlog.tagFilter",
      value: JSON.stringify(config.backlog.tagFilter ?? null),
      type: "string",
      category: "backlog",
      label: "Tag Filter",
      description: "Filter features by this tag name when filterType is 'tag' (case-insensitive)",
      defaultValue: JSON.stringify(null),
      updatedAt: now,
    },
    {
      key: "backlog.epicId",
      value: JSON.stringify(config.backlog.epicId ?? null),
      type: "string",
      category: "backlog",
      label: "Epic Reference",
      description: "Epic reference number when filterType is 'epic' (e.g. PRJ-E-1)",
      defaultValue: JSON.stringify(null),
      updatedAt: now,
    },
    {
